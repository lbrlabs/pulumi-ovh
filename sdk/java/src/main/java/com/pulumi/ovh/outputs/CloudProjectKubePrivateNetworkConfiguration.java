// *** WARNING: this file was generated by pulumi-java-gen. ***
// *** Do not edit by hand unless you're certain you know what you are doing! ***

package com.pulumi.ovh.outputs;

import com.pulumi.core.annotations.CustomType;
import java.lang.Boolean;
import java.lang.String;
import java.util.Objects;

@CustomType
public final class CloudProjectKubePrivateNetworkConfiguration {
    private String defaultVrackGateway;
    private Boolean privateNetworkRoutingAsDefault;

    private CloudProjectKubePrivateNetworkConfiguration() {}
    public String defaultVrackGateway() {
        return this.defaultVrackGateway;
    }
    public Boolean privateNetworkRoutingAsDefault() {
        return this.privateNetworkRoutingAsDefault;
    }

    public static Builder builder() {
        return new Builder();
    }

    public static Builder builder(CloudProjectKubePrivateNetworkConfiguration defaults) {
        return new Builder(defaults);
    }
    @CustomType.Builder
    public static final class Builder {
        private String defaultVrackGateway;
        private Boolean privateNetworkRoutingAsDefault;
        public Builder() {}
        public Builder(CloudProjectKubePrivateNetworkConfiguration defaults) {
    	      Objects.requireNonNull(defaults);
    	      this.defaultVrackGateway = defaults.defaultVrackGateway;
    	      this.privateNetworkRoutingAsDefault = defaults.privateNetworkRoutingAsDefault;
        }

        @CustomType.Setter
        public Builder defaultVrackGateway(String defaultVrackGateway) {
            this.defaultVrackGateway = Objects.requireNonNull(defaultVrackGateway);
            return this;
        }
        @CustomType.Setter
        public Builder privateNetworkRoutingAsDefault(Boolean privateNetworkRoutingAsDefault) {
            this.privateNetworkRoutingAsDefault = Objects.requireNonNull(privateNetworkRoutingAsDefault);
            return this;
        }
        public CloudProjectKubePrivateNetworkConfiguration build() {
            final var o = new CloudProjectKubePrivateNetworkConfiguration();
            o.defaultVrackGateway = defaultVrackGateway;
            o.privateNetworkRoutingAsDefault = privateNetworkRoutingAsDefault;
            return o;
        }
    }
}
